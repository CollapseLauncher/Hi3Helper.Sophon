...
namespace Hi3Helper.Sophon.Helper
{
    internal static class Extension
    {
        ...
        private static readonly byte[] LookupFromHexTable =
        [
        ...
        private static readonly byte[] LookupFromHexTable16 =
        [
            255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
            255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
            255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
...
#else
                    while ((read = streamToUse.Read(buffer, 0, buffer.Length)) > 0)
                    {
                        token.ThrowIfCancellationRequested();
...
#else
        // This is intentional as we don't have an alternative (for now).
        internal static bool TryGetChunkXxh64Hash(
            ReadOnlySpan<char> fileName,
            out byte[]         outHash)
            => fileName.ToString().TryGetChunkXxh64Hash(out outHash);
#endif
...
#pragma warning restore CA1510
...
#else
                while ((read = stream.Read(readBytes, offset, readBytes.Length - offset)) > 0)
                {
                    offset += read;
                }
#endif
...
#if NETSTANDARD2_0_OR_GREATER
        internal static HashSet<T> ToHashSet<T>(this IEnumerable<T> enumerable, IEqualityComparer<T> comparer) => new(enumerable, comparer);
            ...
            where T : IMessage<T>
        {
            ...
            FileInfo localMetadataInfo = new(localMetadataPath);
...
#else
            await manifestProtoStream.CopyToAsync(manifestLocalStream, 64 << 10, innerToken);
#endif
        ...
        internal static FileInfo GetLegacyOrHoyoPlayPatchChunkPath(this SophonPatchAsset asset, string patchOutputDir)
        {
            ...
            ArgumentException.ThrowIfNullOrEmpty(patchOutputDir);
            ArgumentException.ThrowIfNullOrEmpty(asset.PatchNameSource, nameof(asset.PatchNameSource));
#else
            if (asset == null) throw new ArgumentNullException(nameof(asset));
            if (string.IsNullOrEmpty(patchOutputDir)) throw new ArgumentNullException(nameof(patchOutputDir));
            if (string.IsNullOrEmpty(asset.PatchNameSource)) throw new ArgumentNullException(nameof(asset.PatchNameSource));
#endif
...
#else
            string outputDirParent = Path.GetDirectoryName(patchOutputDir);

            // ReSharper disable once StringLiteralTypo
            string hoyoPlayChunkDir  = Path.Combine(outputDirParent ?? "", "ldiff");
            string hoyoPlayChunkPath = Path.Combine(hoyoPlayChunkDir, asset.PatchNameSource);

            string legacyChunkDir  = Path.Combine(outputDirParent ?? "", "chunk_collapse");
            string legacyChunkPath = Path.Combine(legacyChunkDir, asset.PatchNameSource);
#endif
        ...
        internal static int GetFileStreamBufferSize(this long fileSize)
            => fileSize switch
               {
                   // 128 KiB
                   <= 128 << 10 => 4 << 10,
