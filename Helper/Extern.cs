...
{
    private static readonly string _currentProcPath =
        Path.GetDirectoryName(Process.GetCurrentProcess().MainModule?.FileName)?.TrimEnd(Path.DirectorySeparatorChar) ??
        "";
    ...
    private static IntPtr DllImportResolver(string libraryName, Assembly assembly, DllImportSearchPath? searchPath)
    {
        ...
        Console.WriteLine($"[Extern::DllImportResolver] Loading library from path: {libraryName} | Search path: {searchPathName}");
